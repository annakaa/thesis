...
Artifical Neural Networks (ANNs) are inspired by ``real'' Neural Networks - i.e. the human brain and nervous system. They generally consist of neurons, which are nodes that can process inputs and send outputs, and the connections between them. These neurons are grouped in layers: An input layer, an output layer, and a number of hidden layers in between them. Historically, ANNs first had no hidden layers at all; this type of ANN was called ``perceptron''. Later, the addition of one or two hidden layers was introduced and the resulting networks were called ``Multilayer Perceptrons'' (MLPs).
%check this!
Networks with no hidden layers are only able to solve linear problems; the introduction of hidden layers added non-linearities to the calculation. Recent ``Deep'' Neural Networks (DNNs) possess three or more hidden layers, leading to an exponential increase of the degrees of freedom and thus the possibility to model much more complex problems.\\

Classical neurons compute a weighted sum $z$ of their inputs $\textbf{x} = (x_1,x_2,...,x_N)$:
\begin{equation}
z = b + \sum_{i=1}^N x_i w_i = b + \textbf{w}^T \textbf{x}
\end{equation}
%check logit
where $\textbf{w} = (w_1,w_2,...,w_N)$ is a vector of weights, and $b$ is a constant bias. This result is often called the ``logit''. Then, a nonlinear activation function can be applied. In perceptrons, this was the Heaviside step function, shown in figure \ref{fig:activation_heaviside}, resulting in a binary output:
\begin{equation}
y = begin{cases}
1 \text{if} z \leq 0\\
0 \text{otherwise}
...
If connections that skip back are allowed in the network, the network is called a Recursive Neural Network (RNN). These networks are particularly useful for modeling time-dependent series because they have the ability to ``store'' temporal states in their units. However, they were deemed impractical for a long time because they exhibit the exploding/vanishing gradient problem during training \cite{schmidhuber}. This problem was eventually solved with the introduction of memory cells in place of neurons, in particular Long-Short Term Memory (LSTM) units and Gated Recurrent Units (GRU) \cite{}. Nowadays, RNNs are being used successfully in a number of research tasks \cite{}.\\
...
%woanders?
In this work, DNNs with sigmoid units in the hidden layers and a softmax output layer were used in phoneme classification tasks.\\

Neural Network training is performed via the backpropagation algorithm. This algorithm is based on the calculation of a cost function, which computes the difference between the network output and the expected output (e.g. the annotated classes in the training data). When using a softmax output layer, the cross-entropy is usually chosen as the cost function:
\begin{equation}
C = - \sum_{j=1}^M t_j \log y_j
\end{equation}
where $t_j$ is the target value for output unit $j$. Then, the partial derivatives with respect to each logit are calculated:
\begin{equation}
\end{equation}
