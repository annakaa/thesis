...
\begin{figure}
	\begin{center}
		\includegraphics[width=0.8\textwidth]{images/process_training.png}
		\caption{Schematic of the training procedure used in the considered tasks.}
		\label{fig:process_training}
	\end{center}
\end{figure}
\begin{figure}
	\begin{center}
		\includegraphics[width=0.8\textwidth]{images/process_classification}
		\caption{Schematic of the classification procedure used in the considered tasks (the model is the one created during training - see figure \ref{fig:process_training}).}
...
Over time, processing speed of computers increased, and better strategies and hardware for parallel computing became available. This enabled the training of networks with many more layers, allowing for a much better adaptation to many high-dimensional problems \cite{hinton_osindero}. Over the past 10 years, this so-called ``deep learning'' became the state of the art for many machine learning problems \cite{nytimes}\cite{schmidhuber_dl}\cite{deng_hinton_kingsbury}.\\
...
\begin{figure}
	\begin{center}
		\includegraphics[width=0.8\textwidth]{images/process_classification_phones.png}
...
Some work also use a measure called $correct$, which ignores insertions. This makes sense if it is assumed that the phoneme results are used afterwards by an algorithm that is tolerant to insertions. In many cases, they will then also be tolerant to deletions. For cases like this, Hunt suggested a weighted error rate that punishes insertions and deletions less heavily than substitutions \cite{hunt}:
 ...
 \begin{figure}
	\begin{center}
		\includegraphics[width=0.8\textwidth]{images/posteriorgram.png}
...
\begin{figure}
	\begin{center}
		\includegraphics[width=0.8\textwidth]{images/process_training_lid.png}
...
\begin{figure}
	\begin{center}
		\includegraphics[width=0.8\textwidth]{images/process_classification_lid.png}
...
Additionally, a second approach based on phoneme posteriorgrams is tested. Statistics from the posteriorgrams are calculated, and then a second model is trained on these.\\
...
\begin{figure}
	\begin{center}
		\includegraphics[width=0.8\textwidth]{images/process_retrieval.png}
		\caption{Schematic of the procedure for lyrics retrieval.}
		\label{fig:process_retrieval}
	\end{center}
\end{figure}
Lyrics retrieval can be interpreted as an alignment task as well. As shown in figure \ref{fig:process_retrieval}, alignment is performed on a whole database of lyrics instead of just those for a single song. Then, the scores for each alignment are compared, and the best one determines the best match.\\
