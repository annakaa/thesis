...
In this section, three approaches to lyrics-to-audio alignment and retrieval and one practical application were presented. The first one uses HMMs trained on speech for alignment. The mean alignment error in the 2017 \textit{MIREX} challenge was $5.11$s on unaccompanied singing, and $13.96$s and $17.7$s on two polyphonic data sets. A manual check on some examples suggests that the algorithm is already practically usable in many cases, even for polyphonic music. It was also used to generate the annotations for the \textit{DAMP} training data sets as described in section \ref{sec:phonerec_acap}.\\
