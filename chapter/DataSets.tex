...
%aufräumen!!
For comparison, we also tested our algorithm on  the \textit{OGI Multi-language Telephone Speech Corpus (OGIMultilang)} \cite{ogimultilang}, using all recordings for the three previously mentioned languages. This gives us 3,177 utterances in sum with more varying durations (1-60 seconds). For experiments on longer recordings, results on these individual utterances were aggregated for each speaker, producing 118 documents per language (354 in sum).\\



\begin{table}[htdp]
  \caption{{\it Amounts of data in the three used data sets: Sum duration on top, number of utterances in italics.}}
  \begin{center}
...
Unaligned lyrics were retrieved from ???. A thorough semantic analysis can be found in \cite{beghtol}.
