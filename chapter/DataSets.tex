...
\subsection{Hansen's vocal track data set} \label{subsec:data_hansen}
...
\subsection{DAMP data set} \label{sec:data_damp}
...
Since all of these data sets are still much larger than the \textit{TIMIT} speech corpus, another subset of similar size was created for comparison. On the basis of the balanced \textit{DampBB} data set, phoneme instances were discarded until they were balanced and 60,000 frames per phoneme were left, resulting in the \textit{DampBB\_small} data set.\\
For testing new algorithms, two small test data sets were created from the original \textit{DAMP} data set. These are, again, split by gender, and contain one recording per song, resulting in 301 recordings for the female data set and 300 for the male one (since there was one song with not enough available male recordings). They are called \textbf{DampTestF} and \textbf{DampTestM} respectively. For mixed-gender training, testing was simply performed on both data sets.\\
...
\item[DampRetrievalF and DampRetrievalM] {20 hand-picked sung phrases from \textit{DampF} and \textit{DampM} of a few seconds duration with good enunciation and good audio quality. These were selected for fine-tuning retrieval approaches. An overview can be found in appendix \ref{app:retrieval}.}
\end{description}
%Matt's data set??
...
\begin{table}
 ...
 \begin{tabular}{|c||c|c|c|}
  \hline
    & \textbf{Both genders} & \textbf{Female} & \textbf{Male}\\
  \hline
  \hline
 ...
 \end{tabular}\label{tab:damp_corpora}
\end{center}
...
\subsection{Retrieval data set by the author} \label{sec:data_retrieval}
...
\subsection{Hansen's vocal track data set (polyphonic)} \label{subsec:data_hansen_polyphonic}
This data sets consists of the songs in the \textit{ACAP} data set described above in section \ref{subsec:data_hansen}, but with instrumental annotations. The annotations are carried over. This data set is also used for alignment evaluation and denoted as \textit{ACAP\_Poly}.
